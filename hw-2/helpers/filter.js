...
const _validateData = (data, dataScheme) => {
    ...
    for (key in data) {
        ...
        } else if (!data[key] && dataKeyType === 'string') {
            throw new Error(
...
const validateRequest = requestData =>
    _validateData(requestData, requestScheme) &&
    _validateData(requestData.filter, requestScheme.filter) &&
    (!requestData.filter.format || requestData.filter.format === 'csv');

module.exports = { validateRequest, filterUsers };
